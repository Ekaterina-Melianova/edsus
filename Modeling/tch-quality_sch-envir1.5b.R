...
REASONING =~ BSMREA01 + BSMREA02 + BSMREA03 + BSMREA04 + BSMREA05
'
...
sem <- paste0(cfa_higher, formative,
              ...
              REASONING ~ f*Student_Attitudes + BSBGHER 
...
# Run Lavaan:
fit_sem5 <- sem(sem, data = stu_sus_timss, std.lv = T, sampling.weights = 'TOTWGT')
summary(fit_sem5, fit.measures = T, standardized = T)
...
sem_multi <- paste0(cfa_higher, formative,
                    ...
                    REASONING ~ c(f1, f2)*Student_Attitudes + BSBGHER 
...
# By groups
fit_sem_multi5 <- sem(sem_multi, data = stu_sus_timss, std.lv = T, sampling.weights = 'TOTWGT',
                      group = 'traditional_style', meanstructure = TRUE)
summary(fit_sem_multi5, fit.measures = T, standardized = T)
...
sem_multi_plot <- paste0(cfa_higher, formative,
                         ...
                         REASONING ~ Student_Attitudes + BSBGHER 
                         ')

# By groups
fit_sem_multi_plot5 <- sem(sem_multi_plot, data = stu_sus_timss, std.lv = T, sampling.weights = 'TOTWGT',
                           group = 'traditional_style', meanstructure = TRUE)
summary(fit_sem_multi_plot5 , fit.measures = T, standardized = T)

# -----------------------------------------------------------------------------------------------------------------#
# Structural Invariance of Mediation Paths
# -----------------------------------------------------------------------------------------------------------------#

# Teacher_Quality -> Student_Attitudes -> REASONING

sem_1.5 <- paste0(cfa_higher, formative,'
                  # Regressions
                  
                  Student_Attitudes ~ c(a, a)*Teacher_Quality + Comfort_furniture + Safety + Conditions + n_tech + BSBGHER
                  REASONING ~ c(b, b)*Student_Attitudes + BSBGHER
                  ')

fit_sem_1.5 <- sem(sem_1.5, data = stu_sus_timss, std.lv = T, sampling.weights = 'TOTWGT',
                   group = 'traditional_style')
anova_res1.5 <- anova(fit_sem_multi5, fit_sem_1.5)


# Comfort_furniture -> Student_Attitudes -> REASONING
sem_2.5 <- paste0(cfa_higher, formative,'
                  # Regressions
...
fit_sem_2.5 <- sem(sem_2.5, data = stu_sus_timss, std.lv = T, sampling.weights = 'TOTWGT',
                   group = 'traditional_style')
anova_res2.5 <- anova(fit_sem_multi5, fit_sem_2.5)


# Safety -> Student_Attitudes -> REASONING
sem_3.5 <- paste0(cfa_higher, formative,'
                  # Regressions
...
fit_sem_3.5 <- sem(sem_3.5, data = stu_sus_timss, std.lv = T, sampling.weights = 'TOTWGT',
                   group = 'traditional_style')
anova_res3.5 <- anova(fit_sem_multi5, fit_sem_3.5)


# Conditions -> Student_Attitudes -> REASONING
sem_4.5 <- paste0(cfa_higher, formative,'
                  # Regressions
...
fit_sem_4.5 <- sem(sem_4.5, data = stu_sus_timss, std.lv = T, sampling.weights = 'TOTWGT',
                   group = 'traditional_style')
anova_res4.5 <- anova(fit_sem_multi5, fit_sem_4.5)


# n_tech -> Student_Attitudes -> REASONING
sem_5.5 <- paste0(cfa_higher, formative,'
                  # Regressions
...
fit_sem_5.5 <- sem(sem_5.5, data = stu_sus_timss, std.lv = T, sampling.weights = 'TOTWGT',
                   group = 'traditional_style')
anova_res5.5 <- anova(fit_sem_multi5, fit_sem_5.5)
